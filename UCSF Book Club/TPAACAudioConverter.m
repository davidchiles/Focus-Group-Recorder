...
+ (BOOL)AACConverterAvailable {
    ...
    UInt32 encoderSpecifier = kAudioFormatMPEG4AAC_ELD;//kAudioFormatMPEG4AAC;
    UInt32 size;
    ...
    for (UInt32 i=0; i < numEncoders; ++i) {
        if ( encoderDescriptions[i].mSubType == kAudioFormatMPEG4AAC_ELD && encoderDescriptions[i].mManufacturer == kAppleHardwareAudioCodecManufacturer ) {
            _available_set = YES;
            _available = YES;
            return YES;
        }
